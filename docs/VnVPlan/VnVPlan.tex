...
domains. \\
Dr. Jacques Carette & Co-supervisor / Reviewer & Provides feedback on the Drasil
encoding and generated artifacts; co-supervises the project with the goal of
producing a publishable case study. \\
...
Besides the course SRS
checklist\footnote{\url{https://github.com/Xinlu-Y/CAS741/blob/main/docs/Checklists/SRS-Checklist.pdf}},
we will verify the SRS through:
\begin{enumerate}
...
Reviewers apply the VnV
checklist\footnote{\url{https://github.com/Xinlu-Y/CAS741/blob/main/docs/Checklists/VnV-Checklist.pdf}}
to assess feasibility, coverage, and
...
\textbf{Step 6: Drasil code walkthrough.}
The document author will lead a walkthrough of the Drasil encoding with the
supervisor and co-supervisor. The walkthrough will trace how the encoded
knowledge chunks map to the SRS elements (especially IM1), covering input
validation, the numerical integration interface, and the generated module
structure. Findings will be recorded as Drasil issues or project issues.
...
subjected to lightweight static checks (e.g., formatting and linting) and
manual code inspection of the Drasil encoding. In addition, issues identified
during development are tracked in the Drasil issue tracker so that upstream
fixes can be contributed where appropriate. The resulting CI reports, test
outputs, and coverage summaries form the primary evidence that the test
procedures were executed and that the implementation continues to satisfy the
expected behaviors.

\subsection{Software Validation}

Validation asks whether we are building the \emph{right} system. For BSS, true
validation would require comparison with observational or experimental data for
real binary star systems, which is out of scope for this project. Instead,
validation evidence is limited to expert judgement: feedback from the course
instructor and peer reviewers is used to confirm that the chosen assumptions,
inputs/outputs, and reported results align with the goals of the SRS and the
expected use in a learning context. Note that comparisons against reference
solutions and known two-body properties (e.g., center-of-mass conservation,
symmetry) are verification activities and are covered in
Section~\ref{SystemTests}.
...
\subsubsection{Input Handling, Constraint Enforcement, and IVP Solver
Consistency}
The functional test cases in Table~\ref{tab:func-tests} are grouped into two
categories:
\begin{itemize}
  \item \textbf{Input handling and constraint enforcement (T1IVP, T2IVP):}
  These tests verify that the program correctly echoes inputs and rejects
  invalid configurations at the system boundary, \emph{before} the solver is
  invoked.
  \item \textbf{IVP solver and model consistency (T3IVP--T5IVP):} These tests
  exercise the solver and check that the computed trajectories satisfy known
  physical invariants or match reference solutions.
\end{itemize}

All tests use inputs that satisfy the constraints in Table~2 of \citep{SRS}
(except T2IVP, which deliberately violates the center-of-mass constraint). In
particular, the SRS restricts masses to $[10^{29}, 10^{32}]$~kg, positions to
$\lVert\mathbf{r}\rVert \le 10^{13}$~m, velocities to
$\lVert\mathbf{v}\rVert \le 10^{6}$~m/s, and the final time to
$[10^{3}, 10^{10}]$~s; any input outside these bounds is rejected before the
solver runs. The expected outputs are verified against known two-body properties
(e.g., center-of-mass behavior) or reference solutions derived from the instance
models. When applicable, a trusted solver or analytic case is used as a
control.
...
$(2.0\mathrm{E}3,9.0\mathrm{E}3)$, $(-2.5\mathrm{E}3,-1.125\mathrm{E}4)$ &
$1.0\mathrm{E}5$ &
Center-of-mass constraint violation is reported during input validation; the program terminates before entering the solver and no trajectory output is produced. &
C2 \\
\hline
...
\hline
\end{tabularx}
\caption{Automatic Functional Tests: Input Handling and IVP Solver (T1IVP--T5IVP)}
...
\begin{itemize}
  ...
  violates the center of mass consistency constraint (specified in Table~2 of
  the SRS \citep{SRS}, derived from Assumption~A7), then verify that the program
  reports a validation error that is specific and user visible and terminates
  immediately without entering the solver or emitting any trajectory records.

  \item \textbf{C3 (Symmetry and invariants):} For the equal mass symmetric
  configuration, sample $N=2000$ time points $t_k$ over
  $[0, t_{\text{final}}]$. The a~priori error bounds are
  $\epsilon_{cm}$ for center-of-mass drift and $\epsilon_{sym}$ for symmetry
  violation; the test accepts if
  $\max_k \lVert \mathbf{r}_{cm}(t_k)\rVert \le \epsilon_{cm}$ and
  $\max_k \lVert \mathbf{r}_1(t_k)+\mathbf{r}_2(t_k)\rVert \le \epsilon_{sym}$.
  These invariants hold exactly in exact arithmetic; the tolerances account for
  accumulated numerical integration error over the simulation interval.
  ...
  trajectory using the proof-of-concept (PoC) implementation with SciPy's
  DOP853 solver at tight tolerances (\texttt{rtol}$\,=10^{-12}$,
  \texttt{atol}$\,=10^{-14}$), then sample $N=1000$ time points $t_k$ over
  $[0, t_{\text{final}}]$. Accept if
...
high-precision numerical reference produced by SciPy's DOP853 solver at
\texttt{rtol}$\,=10^{-12}$ and \texttt{atol}$\,=10^{-14}$ (see~C5 above),
because the general case does not admit a practical closed-form solution.
...
Note that numerical integration error can accumulate over long simulation
intervals. The test final times are chosen to be short enough (at most a few
orbital periods) so that the solver remains stable; the SRS bound
$t_{\text{final}} \le 10^{10}$ s limits the maximum integration span.
...
\begin{itemize}
  ...
  The COM position is exactly zero in exact arithmetic; over a simulation span
  of $\sim10^{7}$~s at orbital separations of $10^{10}$--$10^{11}$~m, a drift
  of $10^{6}$~m represents $\sim10^{-5}$ relative error, consistent with
  DOP853 at \texttt{rtol}$\,=10^{-10}$.

  \item $\epsilon_{sym}=1.0\mathrm{E}6$ m (symmetry tolerance).
  For the equal-mass case $\mathbf{r}_1+\mathbf{r}_2=\mathbf{0}$ holds
  analytically; the same relative-error reasoning as $\epsilon_{cm}$ applies.

  \item $\epsilon_r^{(4)}=1.0\mathrm{E}4$ m (analytic checkpoint tolerance for
  T4IVP).
  T4IVP uses a circular orbit with $a=10^{10}$~m over one period
  ($\sim1.5\times10^{6}$~s). The short, periodic trajectory limits error
  growth, so a tighter bound of $10^{-6}$ relative to $a$ is feasible.

  \item $\epsilon_r^{(5)}=1.0\mathrm{E}6$ m (reference-solver comparison
  tolerance for T5IVP).
  T5IVP compares the SUT against a reference at \texttt{rtol}$\,=10^{-12}$.
  The looser SUT tolerances and non-circular trajectory lead to less
  predictable error growth, so the wider $10^{6}$~m bound is used.
\end{itemize}
